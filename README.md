...
- 업데이트 도중 Request가 올 수 있는데, 메시지가 유실되지 않기 위해서는 Message Queue를 사용해야 할 수도 있다. 단 건처리가 아닌 대량의 메시지가 한 번에 유입될 수 있다면 Kafka를 사용할 것이지만, 회의실 예약은 단 건으로 요청이 올 거라고 가정하기에 RabbiMQ가 더 적당할 수 있다. 
